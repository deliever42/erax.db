...
module.exports = class MongoDatabase extends EventEmitter {
    ...
    constructor(options) {
        ...
        this.mongo = require('../Schema/MongoSchema')(this.connection, modelName);
    ...
    async set(key, value) {
        ...
        if (!data) {
            ...
            await this.mongo.create({ key: parsedKey, value: json[parsedKey] });
        } else {
            set(json, parsedKey, data.value, this.sep);
            set(json, key, value, this.sep);
            await this.mongo.updateOne({ key: parsedKey }, { value: json[parsedKey] });
        }
