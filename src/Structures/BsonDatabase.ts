...
export class BsonDatabase<V> extends BaseDatabase<V> {
    public options: BsonDatabaseOptions;
    private backupInterval: any = null;
    public constructor(
        options: BsonDatabaseOptions = {
            space: 4,
            cache: true,
            filePath: join(process.cwd(), 'database.bson'),
            backup: {
                enabled: false
            }
        }
        ...
        for (const path of paths) {
            ...
            if (!existsSync(resolvedFilePath) && extname(resolvedFilePath) !== '.bson')
                mkdirSync(resolvedFilePath);
            else if (!existsSync(resolvedFilePath) && extname(resolvedFilePath) === '.bson') {
                _.write<V>('bson', resolvedFilePath, {}, this.options.space);
                break;
            }
        }
        ...
        if (this.options.cache) this.cache = _.read('bson', resolvedFilePath);
        ...
        if (this.options.backup!.enabled && this.options.backup!.filePath) {
            this.backupInterval = setInterval(() => {
                ...
                if (this.options.cache) {
                    ...
                    const file = _.read<V>('bson', this.options.backup!.filePath!);
    ...
    public set(key: string, value: V | Array<V>): V | Array<V> {
        ...
        if (this.options.cache) {
            ...
            _.write<V>('bson', this.options.filePath, this.cache, this.options.space);
        } else {
            const file = _.read<V>('bson', this.options.filePath);
            set(file, key, value);
            _.write<V>('bson', this.options.filePath, file, this.options.space);
        }
    ...
    public clear() {
        this.cache = {};
        return _.write<V>('bson', this.options.filePath, {}, this.options.space);
    }
